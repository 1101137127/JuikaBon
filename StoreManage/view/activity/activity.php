<?php
  ...
  $store_id = $_SESSION['store_id'];
	$stmt = $conn->prepare("SELECT `act_id`,`act_name`  FROM `activity` where use_flag='Y' and `store_id`='$store_id' ");
	$stmt->execute();
	$list.='';
	foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
		$list.='<li>
		<a href="activityDetail.php?act_id='.$row['act_id'].'" data-ajax="false"><h2>' . $row['act_name'] . '</h2></a>
    <a onclick="doConfirm('.$row['act_id'].')"  id="doConfirm" data-ajax="false">Delete</a>
    </li>';
	}
...
<div data-role="page" id="pageone" >
   <!-- Pageone header -->
    ...
    <div style="background:#001F33;border:0px;height:50px;">
      <br/>
      <div style="font-size:22px;color:white;text-align:center;">活動管理</div>
    </div>
  <!-- End Pageone header -->
  ...
  <div data-role="main" class="ui-content" id="content" style="margin:0 auto;height:650px;background:white;">
	<div id='member-main-downcontent' class='member-main col-lg-12 col-md-12 col-sm-12 col-xs-12'>
            <div id='' style="margin:0 auto;width:100%">
              <form class="ui-filterable">
                <input id="myFilter" data-type="search" placeholder="搜尋活動">
              </form>
              <div id="data-content">
    ...
    <div>
    <a href="activity_Insert.html" data-ajax="false" class="ui-btn ui-btn-b ui-icon-plus ui-btn-icon-left ui-btn-inline ui-mini">新增</a>
    </div>
