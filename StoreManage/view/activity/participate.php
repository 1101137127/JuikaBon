...
header("Content-Type:text/html; charset=utf-8");
  ...
  $msg = "";
	$msg .= '<input type="hidden"  id="act_id" value="'.$row['act_id'].'"  />';
  $show_count="";
  $show_count='<div><h4>目前參加人數:'.$total_count.'人</h4></div>';
  ...
  function getDetail (act_id){
    $.ajax({
        ...
        success: function(data){
          if(data =="Null"){
            alert("目前尚無人參加此活動");
            var link = "activityDetail.php?act_id="+act_id;
            location.href = link;
          }
          else{
...
<div data-role="page" id="pageone" >
   <!-- Pageone header -->
    ...
    <div style="background:#001F33;border:0px;height:50px;">
      <br/>
      <div style="font-size:22px;color:white;text-align:center;">活動參與狀況</div>
    </div>
  <!-- End Pageone header -->
	...
	<div id='member-main-downcontent' class='member-main col-lg-12 col-md-12 col-sm-12 col-xs-12'>
            <div id='' style="margin:0 auto;width:100%">
              ...
              <table data-max="30" data-min="11" data-position="fixed" id="detail" data-role="table" data-mode="columntoggle" class="ui-responsive table-stroke" border="1">
                <thead data-position="fixed"><tr><th width=5%>參與序號</th><th width=5%>會員編號</th><th width=15%>會員姓名</th><th width=15%>參與日期</th></tr></thead>
                <tbody></tbody>
              </table>
              <?php echo $show_count ?>
            </div>
