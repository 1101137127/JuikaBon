<?php
	...
	include("../connectDBModel/db.php");
        $id=$_GET['id'];
	$stmt = $conn->prepare("SELECT * FROM `message_Test` where mess_id='".$id."'");
	$stmt->execute();
        $msg.='';
	...
	$msg.='
                ...
                <div data-role="fieldcontain" >
					<label class="Inputlabel" for="mess_name" >簡訊名稱:</label>
					<input type="text" name="mess_name" id="mess_name" value="' . $row['mess_name'] . '"  />					
		</div>
                <div data-role="fieldcontain" >
					<label class="Inputlabel" for="mess_text" >簡訊內容:</label>
					<input type="text" name="mess_text" id="mess_text" value="' . $row['mess_text'] . '"  />					
		</div>
