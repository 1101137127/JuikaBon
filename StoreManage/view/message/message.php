<?php
  ...
  $stmt = $conn->prepare("SELECT `mess_id`,`mess_name` FROM `message` where use_flag='Y' and `store_id`='$store_id'");
  $stmt->execute();
  $list.='';
  ...
  foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
    $list.='<li>
    <a href="messageDetail.php?mess_id='.$row['mess_id'].'" id="doConfirm" data-ajax="false"><h2>' . $row['mess_name'] . '</h2></a>
    <a onclick="doConfirm('.$row['mess_id'].')"  id="doConfirm" data-ajax="false">Delete</a>
    </li>';
  }
...
<div data-role="page" id="pageone" >
  ...
  <div data-role="header" style=" background:#0097A7;border:0px;height:80px;">
    ...
    <div style="background:#001F33;border:0px;height:50px;">
      <br/>
      <div style="font-size:22px;color:white;text-align:center;">訊息推播</div>
    </div>
  <!-- End Pageone header -->
  ...
  <div id='member-main-downcontent' class='member-main col-lg-12 col-md-12 col-sm-12 col-xs-12'>
            <div id='' style="margin:0 auto;width:100%">
              <form class="ui-filterable">
                <input id="myFilter" data-type="search" placeholder="搜尋訊息">
              </form>
              <div id="data-content">
    ...
    <div>
    <a href="messageInsert.html" data-ajax="false" class="ui-btn ui-btn-b ui-icon-plus ui-btn-icon-left ui-btn-inline ui-mini">新增</a>
