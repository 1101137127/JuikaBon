<?php
  ...
  $store_id = $_SESSION['store_id'];
	$stmt = $conn->prepare("SELECT `qst_id`,`qst_name` FROM `questionnaire` where use_flag='Y' and `store_id`='$store_id'");
	$stmt->execute();
	$list.='';
	foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
    $list.='<li>
    <a href="questionnaireDetail.php?qst_id='.$row['qst_id'].'" data-ajax="false"><h2>' . $row['qst_name'] . '</h2></a>
    <a onclick="doConfirm('.$row['qst_id'].')"  id="doConfirm" data-ajax="false">Delete</a>
    </li>';
  }
...
<link href="../../jquery-mobile-css/carousel.css" rel="stylesheet">
  ...
  <script>
    ...
    function doConfirm(id) {
      ...
      if(del==true){
        $.ajax({
        url: "./model/Delete_questionnaire.php", 
        type:"POST",
        data: {
          qst_id:id
        },
...
<div data-role="page" id="pageone" >
   <!-- Pageone header -->
    ...
    <div style="background:#001F33;border:0px;height:50px;">
      <br/>
      <div style="font-size:22px;color:white;text-align:center;">問卷管理</div>
    </div>
  <!-- End Pageone header -->
  ...
  <div data-role="main" class="ui-content" id="content" style="margin:0 auto;height:650px;background:white;">
	<div id='member-main-downcontent' class='member-main col-lg-12 col-md-12 col-sm-12 col-xs-12'>
            <div id='' style="margin:0 auto;width:100%">
              <form class="ui-filterable">
                <input id="myFilter" data-type="search" placeholder="搜尋問卷">
              </form>
    ...
    <div>
    <a href="questionnaire_Insert.html" data-ajax="false" class="ui-btn ui-btn-b ui-icon-plus ui-btn-icon-left ui-btn-inline ui-mini">新增</a>
    </div>
